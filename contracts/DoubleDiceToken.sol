 ...
 *              /o_____o/o   \    o\   o/
 *              \o      \   o/      \o /
 *  ______     __\   o   \  /\o______\/       _____     ____    ____    ____   _______
 * |  __  \   /   \______o\/ |  _ \  | |     |  ___|   |  _ \  |_  _|  / ___| |   ____|
 * | |  \  | | / \ | | | | | | |_| | | |     | |_      | | \ |   ||   | /     |  |
 * | |   | | | | | | | | | | |  _ <  | |     |  _|     | | | |   I|   | |     |  |__
 ...
 * @author DoubleDice Team <team@doubledice.com>
 * @notice ERC-20 token extended with special yield functionality.
 *
 ...
 *
 * It is not possible to mint new DODI on top of the minted 10 billion DODI.
 *
 * The DODI on the `UNDISTRIBUTED_YIELD_ACCOUNT` is controlled by the `owner()` of this contract,
 * who may choose to:
 * - Distribute a portion or all of the remaining undistributed yield to token holders
 * - Burn a portion or all of the remaining undistributed yield, thus decreasing the total DODI supply
 *
 ...
 * Token holders may then `claimYield()` to transfer their received yield
...
contract DoubleDiceToken is
    ...
    Ownable
{
    /// @notice Account holding the portion the 3.7 billion DODI that have not yet been distributed or burned by `owner()`
    address constant public UNDISTRIBUTED_YIELD_ACCOUNT = 0xD0D1000000000000000000000000000000000001;

    /// @notice Account holding yield that has been distributed but not yet claimed by its owner
    ...
    /// @dev Holds account-specific data
    struct AccountEntry {
    ...
    /// @dev The state for a token-holding account is stored in this mapping in conjuction with
    ...
    /// The larger the value of `ONE`, the smaller the rounding errors in this contract.
    ...
    /// @dev This is the single most important value in this contract.
    /// Yield distribution to all accounts is recorded by increasing (eagerly) this contract-wide `_factor`,
    ...
    /// @notice Query contract for total yield token amount claimable by `account`.
    ...
    /// @dev If yield has been distributed since the last time `account` was:
    /// - either party to a `transfer` or `transferFrom`,
    /// - or was excluded from a `distributeYield`,
    /// then this function will:
    /// 1. recalculate `unclaimedYieldOf(account)` and reflect the result
    /// 2. write (or "capture") the result of this calculation in `entry.capturedUnclaimedYield`
    /// 3. update the value of `entry.factorAtCapture` to `_factor`
    /// @custom:emits-event UnclaimedYieldIncrease if calculation results in an increase in `capturedUnclaimedYield`
    ...
    /// @custom:reverts-with "Reserved account" if `excludedAccounts` contains
    /// `UNDISTRIBUTED_YIELD_ACCOUNT` or `UNCLAIMED_DISTRIBUTED_YIELD_ACCOUNT`
    ...
    /// @notice Calls `claimYieldFor` the caller.
